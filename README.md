...
Tube Cleaner is not listed there because it is intended only for iOS Safari and must run as a normal `.user.js` userscript.
...
Tube Cleaner Lite is sourced from [wBlock-userscripts](https://github.com/0xCUB3/wBlock-userscripts/tree/main/packages/tube-cleaner). The version in this collection is intended only for iOS Safari and must run as a userscript; it has no Brave Custom Scriptlets rule.
